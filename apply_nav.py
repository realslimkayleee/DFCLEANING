...
# 1. Write js/header.js
header_js = """class SiteHeader extends HTMLElement {
  connectedCallback() {
...
/* Ensure mobile menu works inside the fixed header */
...
if '.site-header-fixed' not in css_content:
    with open('css/style.css', 'a', encoding='utf-8') as f:
        f.write("\n" + css_addition)


# 3. Replace HTML in all files
html_files = []
...
for path in html_files:
    ...
    path = path.replace('\\', '/')
    is_nested = path.startswith('./services/') or path.startswith('services/')
    
    root_prefix = '../' if is_nested else ''
    
    with open(path, 'r', encoding='utf-8') as f:
        content = f.read()
        
    # Check if we already injected header.js
    if 'js/header.js' not in content:
        content = content.replace('</head>', f'  <script src="{root_prefix}js/header.js" defer></script>\n</head>')
        
    # Find header block
    # It starts with <!-- HEADER / NAVIGATION --> or <header> and ends with </header>
    start_idx = content.find('<!-- HEADER / NAVIGATION -->')
    if start_idx == -1:
    ...
    if start_idx != -1:
        ...
        if end_idx != -1:
            end_idx += len('</header>')
            replacement = f'<site-header root-path="{root_prefix}"></site-header>'
            content = content[:start_idx] + replacement + content[end_idx:]
            
            with open(path, 'w', encoding='utf-8') as f:
                f.write(content)

print("Nav bar applied to all HTML files.")
